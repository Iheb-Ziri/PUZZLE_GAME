    ...
    void init_bg(Enigme2 *bg) {
        ...
        bg->font = TTF_OpenFont("Pixel-Game.otf", 70);
        if (!bg->font) {
        ...
        bg->pos_btn_text.x = 450;
        bg->pos_btn_text.y = 880;
        bg->pos_btn_text.w = bg->surfaceTexte->w;
        bg->pos_btn_text.h = bg->surfaceTexte->h;

        bg->show_gamerule = 1;

        bg->img_win2 = IMG_Load("images/win.png");
        bg->img_lose2 = IMG_Load("images/loose.png");

        bg->pos_win_lose2.x = 570;
        bg->pos_win_lose2.y = -10;
    ...
    void afficher_bg1(Enigme2 *bg, SDL_Surface *ecran) {   
        if (bg->show_gamerule) {
            ...
            if (wakt_tawa - bg->gamerule_start_time < 7000) {
        ...
        if (bg->timer[bg->compteur])
            ...
            if (bg->jouer_gagner == 1) {
                float angle = 0;
                float angle_step = 360.0 / 190; 
                int delay = 10;
                SDL_Surface *image_rotate;
                SDL_Rect pos_img_rotate = {570, -10};
                ...
                while (bg->win_i < 200) {
                    ...
                    if (angle < 360) {
                        image_rotate = rotozoomSurface(bg->img_win2, angle, 1, 1);
                        SDL_BlitSurface(image_rotate, NULL, ecran, &pos_img_rotate);
        ...
        if (bg->afficher_loose == 1) {
            float angle = 0;
                ...
                while (bg->loose_i < 200) {
                    SDL_FillRect(ecran, NULL, SDL_MapRGB(ecran->format, 0, 0, 0)); 
                    for (int i = 0; i < 27; i++) {
                        SDL_BlitSurface(bg->puzzle[i], NULL, ecran, &bg->pos_btn_puzzle[i]);
                    }
                    if (bg->loose[bg->loose_i]) {
                        SDL_BlitSurface(bg->loose[bg->loose_i], NULL, ecran, &bg->pos_loose);
                    }
                    bg->loose_i++;
                    if (angle < 360) {
                        image_rotate = rotozoomSurface(bg->img_lose2, angle, 1, 1);
                        SDL_BlitSurface(image_rotate, NULL, ecran, &pos_img_rotate);
                        SDL_FreeSurface(image_rotate);
                        angle += angle_step;
                    }
...
void init_bg2(Enigme2_2 *bg) {
    ...
    bg->pos_btn_text.x = 460;
    bg->pos_btn_text.y = 135;
    bg->pos_btn_text.w = bg->surfaceTexte->w;
    bg->pos_btn_text.h = bg->surfaceTexte->h;
...
void afficher_bg2(Enigme2_2 *bg, SDL_Surface *ecran) {
    ...
    if (bg->afficher_loose == 1) {
        float angle = 0;
            float angle_step = 360.0 / 190;  // pour que ça dure exactement 200 frames
            int delay = 10;
            SDL_Surface *image_rotate;
            SDL_Rect pos_img_rotate = {570, -10};
            ...
            while (bg->loose_i < 200) {
                ...
                SDL_FillRect(ecran, NULL, SDL_MapRGB(ecran->format, 0, 0, 0)); // nettoyer écran
...
void init_bg3(Enigme2_3 *bg) {
    ...
    bg->pos_btn_text.x = 440;
    bg->pos_btn_text.y = 860;
    bg->pos_btn_text.w = bg->surfaceTexte->w;
    bg->pos_btn_text.h = bg->surfaceTexte->h;
